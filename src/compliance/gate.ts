...
import { evaluateLocality, nextOpenAt, type LocalityEvaluation } from './calling-window.js';
...
function windowReasons(
  evaluations: LocalityEvaluation[],
  retryableAt: Date | undefined
): DenyReason[] {
  ...
  for (const e of evaluations) {
    if (e.open) continue;
    const where = `${e.locality.jurisdiction} (${e.localTime})`;
    ...
    } else if (!e.statutoryOpen) {
      add('OUTSIDE_STATUTORY_WINDOW', `${where}: ${e.weekday} is outside the legal calling window`);
    } else {
      add('OUTSIDE_POLICY_WINDOW', `${where}: ${e.weekday} is outside the operator's calling window`);
    }
  }
...
export function evaluateDialRequest(
  ...
  const evaluations = locality.candidates.map((c) => evaluateLocality(at, c, request.market, policy, calendar));
  if (evaluations.some((e) => !e.open)) {
    ...
    const timingProblem = evaluations.some(
      (e) => !e.open && e.holiday.kind !== 'out-of-coverage' && e.holiday.kind !== 'unverified'
    );
    const nextOpen = timingProblem
      ? nextOpenAt(at, locality.candidates, request.market, policy, calendar)
      : null;
    reasons.push(...windowReasons(evaluations, nextOpen ?? undefined));
  }
  ...
  const evidence: DecisionEvidence = {
    ...
    number,
    locality,
    localTimes: evaluations.map((e) => ({
      jurisdiction: e.locality.jurisdiction,
      timezone: e.locality.timezone,
      local: e.localTime
    })),
